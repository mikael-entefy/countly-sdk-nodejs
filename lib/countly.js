 ...
 */
	Countly.init = function(ob){
		if(!inited){
			...
			Countly.url = handleConfigUrl(ob.url || Countly.url || "");
	...
	**/
    ...
    function handleConfigUrl (urlString) {

      urlString = stripTrailingSlash(urlString);

      let urlObj = url.parse(urlString);
      if (urlObj.pathname) {
        apiPath = urlObj.pathname + apiPath;
        urlObj.pathname = '';
      }

      if (urlObj.search) {
        throw new Error("There can't be a querystring in the base url");
      }
